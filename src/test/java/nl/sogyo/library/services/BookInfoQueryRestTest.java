...
public class BookInfoQueryRestTest extends JerseyTest {
	...
	public void getBookInfoOfProRestfulApis() {
		String output = target("/book").path("46").request().header(ContainerRequest.AUTHORIZATION, TEST_ID_TOKEN_2).get(String.class);
	...
	public void requestForNotExistingIdGivesEmptyResults() {
		String output = target("/book").path("10000000").request().header(ContainerRequest.AUTHORIZATION, TEST_ID_TOKEN_2).get(String.class);
